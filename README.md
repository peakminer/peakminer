...
```bash
wget -q https://github.com/peakminer/peakminer/releases/download/v2.16.5/peakminer-2.16.5-linux-x86_64 -O peakminer && \
chmod +x peakminer && \
...
# high-performance GPU miner · v2.16.5
(c) 2026 PeakMiner — proprietary, all rights reserved; no reverse engineering / redistribution (see LICENSE). build=20260921-8a0bf1
...
| Field | Value |
|---|---|
| Installation URL | `https://github.com/peakminer/peakminer/releases/download/v2.16.5/peakminer-2.16.5.tar.gz` |
...
```bash
docker pull peakminer/peakminer:2.16.5

# Run — -t shows the live miner output
docker run --rm -t --gpus all peakminer/peakminer:2.16.5 \
...
docker build -t peakminer:2.16.5 --build-arg PEAKMINER_VERSION=2.16.5 .
...
```bash
docker run --rm -t --gpus all -p 4068:4068 peakminer/peakminer:2.16.5 \
