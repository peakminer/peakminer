...
(c) 2026 PeakMiner — proprietary, all rights reserved; no reverse engineering / redistribution (see LICENSE). build=20260925-2ca0eb
...
Behavior:
  ...
  -a, --api-port <[host:]port>
      ...
      --cmp-unlock [<CMP_UNLOCK>]
          pearl, Linux: unlock CMP HX (40HX / 90HX / 170HX) compute performance. Off by default.
          When set, the first root run provisions a patched NVIDIA open driver for the running
          kernel
...
| sm_75 | turing | T4 / CMP 30HX / **CMP 40HX** (driver unlock via `--cmp-unlock` — Pearl, Linux, needs root) / **RTX 20xx (optimized)** · GTX 16xx — not for Pearl |
| sm_80 / sm_86 | ampere | A100 / **CMP 170HX** / **CMP 90HX** (driver unlock via `--cmp-unlock` — Pearl, Linux, needs root) / **RTX 30xx (optimized)** |
