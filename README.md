...
Latest full sweep: **v2.1.0**, 20 GPUs, 0 invalid shares. Measured on vast.ai at **default OC** —
...
| Field | Value |
|---|---|
| Installation URL | `https://github.com/peakminer/peakminer/releases/download/v2.1.0/peakminer-2.1.0.tar.gz` |
...
```bash
docker pull peakminer/peakminer:2.1.0

# Run — -t shows the live miner output
docker run --rm -t --gpus all peakminer/peakminer:2.1.0 \
...
docker build -t peakminer:2.1.0 --build-arg PEAKMINER_VERSION=2.1.0 .
...
```bash
docker run --rm -t --gpus all -p 4068:4068 peakminer/peakminer:2.1.0 \
